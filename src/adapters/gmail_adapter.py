    ...
    def _get_credentials(self) -> Credentials:
        ...
        if token_path.exists():
            try:
                ...
                logger.info(f"Loaded existing Gmail OAuth credentials from {token_path}")
            except Exception as e:
                logger.warning(f"Failed to load token from {token_path}: {e}. Initiating re-auth.")
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                try:
                    ...
                    logger.error(f"Failed to refresh OAuth token: {e}. Re-authenticating...")
                    creds = None
            ...
            if not creds:
                ...
                if "dummy" in settings.gmail_client_id:
                    logger.warning("Using dummy credentials. OAuth flow will fail if run interactively.")
                
                logger.info("Starting local Gmail OAuth2 InstalledAppFlow redirect server...")
    ...
    def apply_action(
        ...
        try:
            ...
            if action == EmailAction.DELETE:
                ...
                logger.info(f"Email {email_id} tagged with label '{label_name}' and archived")
            ...
            logger.error(f"Gmail API HTTP error applying action {action} to {email_id}: {error}")
        except Exception as error:
            logger.error(f"Unexpected error applying action {action} to {email_id}: {error}")
